...
namespace
{
	...
	static DXGI_FORMAT ShaderDataTypeToDx11BaseType(pg::ShaderDataType type)
	{
		switch (type)
		{
		case pg::ShaderDataType::Float:    return DXGI_FORMAT_R32_FLOAT;
		case pg::ShaderDataType::Float2:   return DXGI_FORMAT_R32G32_FLOAT;
		case pg::ShaderDataType::Float3:   return DXGI_FORMAT_R32G32B32_FLOAT;
		case pg::ShaderDataType::Float4:   return DXGI_FORMAT_R32G32B32A32_FLOAT;
			//case pg::ShaderDataType::Mat3:    
			//case pg::ShaderDataType::Mat4:     
		case pg::ShaderDataType::Int:      return DXGI_FORMAT_R32_SINT;
		case pg::ShaderDataType::Int2:     return DXGI_FORMAT_R32G32_SINT;
		case pg::ShaderDataType::Int3:     return DXGI_FORMAT_R32G32B32_SINT;
		case pg::ShaderDataType::Int4:     return DXGI_FORMAT_R32G32B32A32_SINT;
		case pg::ShaderDataType::Bool:     return DXGI_FORMAT_R32_UINT;
		}
	...
	static void BufferLayoutToDx11InputDesc(const pg::BufferLayout& bufferLayout, std::vector<D3D11_INPUT_ELEMENT_DESC>& layoutDesc)
	{
	...
	pg::BufferElement ParseBufferElement(const ELayoutType& typeSrc, const std::string& identifierSrc)
	{
		switch (typeSrc)
		{
		case ELayoutType::Float4:
			return { pg::ShaderDataType::Float4, identifierSrc };
		case ELayoutType::Float3:
			return { pg::ShaderDataType::Float3, identifierSrc };
		case ELayoutType::Float2:
			return { pg::ShaderDataType::Float2, identifierSrc };
		case ELayoutType::Float:
			return { pg::ShaderDataType::Float, identifierSrc };
		case ELayoutType::Int:
		default:
			return { pg::ShaderDataType::Int, identifierSrc };
		}
	}

	static pg::BufferLayout ParseLayoutFromString(const std::string& source)
	{
		std::vector<pg::BufferElement> elements;
		...
		return pg::BufferLayout(elements);
	}
}

pg::Dx11Shader::Dx11Shader(const std::string& filepath)
{
...
pg::Dx11Shader::Dx11Shader(const std::string& name, const char* vertexSrc, const char* fragmentSrc, const char* buffLayout)
{
...
std::string pg::Dx11Shader::ReadFile(const std::string& filepath)
{
	std::string result;
...
std::unordered_map<unsigned int, std::string> pg::Dx11Shader::PreProcess(const std::string& source)
{
...
void pg::Dx11Shader::Compile(const std::unordered_map<unsigned int, std::string>& shaderSources)
{
	pg::RAII_PtrRelease<ID3D10Blob> errorBlob;
	pg::RAII_PtrRelease<ID3D10Blob> vsBlob;
	pg::RAII_PtrRelease<ID3D10Blob> psBlob;
...
DirectX::XMMATRIX pg::Dx11Shader::ConvertGLMMatrixToDX(const glm::mat4& glmMatrix)
{
...
void pg::Dx11Shader::Unbind() const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	ID3D11DeviceContext* deviceContext = context->GetPd3dDeviceContext();
...
void pg::Dx11Shader::UploadUniformInt(const std::string& name, int value) const
{
	...
	pg::Dx11ConstantBuffer<IntBufferType> buffer(&vectorData);
...
void pg::Dx11Shader::UploadUniformFloat(const std::string& name, float value) const
{
	...
	pg::Dx11ConstantBuffer<FloatBufferType> buffer(&vectorData);
	buffer.Bind(GetConstantBufferSlot(name));
}
...
void pg::Dx11Shader::UploadUniformFloat2(const std::string& name, const glm::vec2& value) const
{
	...
	pg::Dx11ConstantBuffer<Vector2BufferType> buffer(&vectorData);
	buffer.Bind(GetConstantBufferSlot(name));
}
...
void pg::Dx11Shader::UploadUniformFloat3(const std::string& name, const glm::vec3& value) const
{
	...
	pg::Dx11ConstantBuffer<Vector3BufferType> buffer(&vectorData);
	buffer.Bind(GetConstantBufferSlot(name));
}
...
void pg::Dx11Shader::UploadUniformFloat4(const std::string& name, const glm::vec4& value) const
{
	...
	pg::Dx11ConstantBuffer<Vector4BufferType> buffer(&vectorData);
	buffer.Bind(GetConstantBufferSlot(name));
}

void pg::Dx11Shader::UploadUniformMat3(const std::string& name, const glm::mat3& matrix) const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	ID3D11DeviceContext* deviceContext = context->GetPd3dDeviceContext();

	DirectX::XMMATRIX mat = ConvertGLMMatrixToDX(matrix);
	pg::Dx11ConstantBuffer<DirectX::XMMATRIX> buffer(&mat);
	buffer.Bind(GetConstantBufferSlot(name));
}

void pg::Dx11Shader::UploadUniformMat4(const std::string& name, const glm::mat4& matrix) const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	ID3D11DeviceContext* deviceContext = context->GetPd3dDeviceContext();
