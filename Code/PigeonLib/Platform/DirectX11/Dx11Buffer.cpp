...
pg::Dx11VertexBuffer::Dx11VertexBuffer(const float* vertices, uint32_t size, uint32_t stride)
{
...
void pg::Dx11VertexBuffer::Bind() const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	const UINT offset = 0;
...
void pg::Dx11VertexBuffer::Unbind() const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	const UINT offset = 0;
	const UINT stride = 0;
...
void pg::Dx11VertexBuffer::SetVertices(const float* vertices, unsigned int count, unsigned int countOffset)
{
...
void pg::Dx11VertexBuffer::AppendVertices(const float* vertices, unsigned int count, unsigned int countOffset)
{
	D3D11_MAPPED_SUBRESOURCE mappedResource;
...
pg::Dx11IndexBuffer::Dx11IndexBuffer(const uint32_t* indices, uint32_t count)
{
...
void pg::Dx11IndexBuffer::Bind() const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	context->GetPd3dDeviceContext()->IASetIndexBuffer(m_Data.m_Buffer.get(), DXGI_FORMAT_R32_UINT, 0);
}

void pg::Dx11IndexBuffer::Unbind() const
{
	auto context = static_cast<pg::Dx11Context*>(pg::Application::Get().GetWindow().GetGraphicsContext());
	context->GetPd3dDeviceContext()->IASetIndexBuffer(nullptr, DXGI_FORMAT_UNKNOWN, 0);
}

void pg::Dx11IndexBuffer::SetIndices(const uint32_t* indices, unsigned int count, unsigned int countOffset)
{
	D3D11_MAPPED_SUBRESOURCE mappedResource;
...
void pg::Dx11IndexBuffer::AppendIndices(const uint32_t* indices, unsigned int count, unsigned int countOffset)
{
	D3D11_MAPPED_SUBRESOURCE mappedResource;
