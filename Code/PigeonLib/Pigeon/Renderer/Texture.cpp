...
pig::U_Ptr<pig::Texture2DArray> pig::Texture2DArray::Create(unsigned int count)
{
	switch (pig::Renderer::GetAPI())
	{
	case pig::RendererAPI::API::None:    PG_CORE_ASSERT(false, "RendererAPI::None is currently not supported!"); return nullptr;
	case pig::RendererAPI::API::DirectX11:  return std::make_unique<pig::Dx11Texture2DArray>(count);
	}
