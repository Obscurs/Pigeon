...
pg::OrthographicCamera::OrthographicCamera(float left, float right, float bottom, float top)
{
...
void pg::OrthographicCamera::SetProjection(float left, float right, float bottom, float top)
{
	m_Data.m_ProjectionMatrix = glm::ortho(left, right, bottom, top, -1.0f, 1.0f);
	m_Data.m_ViewProjectionMatrix = m_Data.m_ProjectionMatrix * m_Data.m_ViewMatrix;
}

void pg::OrthographicCamera::RecalculateViewMatrix()
{
