...
{
	DrawLayerBatch(transform, col, pig::Renderer2D::s_DefaultTexture, glm::vec4(0.f, 0.f, 1.f, 1.f), origin);
}

void pig::Renderer2D::DrawSprite(const pig::Sprite& sprite)
{
	DrawLayerBatch(sprite.GetTransform(), glm::vec3(1.f), sprite.GetTextureID(), sprite.GetTexCoordsRect(), sprite.GetOrigin());
}
...
void pig::Renderer2D::DrawString(const glm::mat4& transform, const std::string& string, pig::S_Ptr<pig::Font> font, const glm::vec4& color, float kerning, float linespacing)
{
	...
	for (size_t i = 0; i < string.size(); i++)
	{
		...
		if (font->IsCharacterDrawable(character))
		{
			...
			DrawLayerBatch(charTransform, color, font->GetFontID(), texCoords, originSprite);
		}
...
void pig::Renderer2D::DrawQuad(const glm::mat4& transform, const pig::UUID& textureID, const glm::vec3& origin)
{
	...
	DrawLayerBatch(transform, glm::vec3(1.f), textureID, glm::vec4(0.f, 0.f, 1.f, 1.f), origin);
}
...
void pig::Renderer2D::DrawLayerBatch(const glm::mat4& transform, const glm::vec3& col, const pig::UUID& textureID, glm::vec4 texRect, const glm::vec3& origin)
{
	if (s_Data.m_TextureMap.find(textureID) != s_Data.m_TextureMap.end())
	{
		...
		std::unordered_map<pig::UUID, pig::Renderer2D::Data::BatchData>& texBatches = s_Data.m_LayerBatchMap[layer];
		pig::Renderer2D::Data::BatchData& texBatch = texBatches[textureID];
...
void pig::Renderer2D::Flush()
{
	...
	for (int i = 0; i < 100; i++)
	{
		auto& layer = s_Data.m_LayerBatchMap.find(i);
		if (layer != s_Data.m_LayerBatchMap.end())
		{
			std::unordered_map<pig::UUID, pig::Renderer2D::Data::BatchData>& texBatches = layer->second;
