...
| Checkpoint | A complete trained diffusion model file (`.safetensors`/`.ckpt`), the base weights generation runs against. The engine targets the **SD 1.5** model family; the family is a config field, but a single generation is coherent only within one family (an SD 1.5 LoRA/ControlNet does not apply to an SDXL Checkpoint). Declared in the Resource Manifest and loaded into VRAM **once at startup, resident for the whole session** — **one Checkpoint per session** (switching requires relaunch). |
...
| Diffusion Job | One in-flight generation tracked by the diffusion subsystem: its progress (current denoising step of total) and completion state, exposed so the app can show a loading/progress state. Runs on a background worker so the frame loop never blocks; many frames elapse during a single job. |
