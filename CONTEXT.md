...
| Text Gen Backend | The platform-abstracted LLM inference runtime (`pg::TextGenBackend`): `LoadModel(path)` loads the session-resident model once, `IsLoaded()` reports readiness, `Generate(params)` runs one prompt→completion loop synchronously (on a worker thread) and returns the text (empty on failure). `Create()` selects the concrete backend by the renderer-API switch (mirroring AudioDevice / DiffusionBackend): **LlamaCppBackend** (real, wraps `llama.cpp`, active only under `-DPG_ENABLE_TEXTGEN=ON`; inert no-op otherwise) or **TestingTextGenBackend** (deterministic mock for tests). Runs on the **CPU** (no CUDA) so it never contends with the GPU the renderer and Diffusion share. |
